...
/// --------------------------------------------------------------------
pub fn init_local_cache(interfaces: &FlodbaddInterfaces) {
    ...
    for interface in interfaces.iter() {
        for addr in &interface.ipv6 {
            let (ip, prefix) = match addr {
                FlodbaddInterfaceAddrTypeV6::Temporary(info)
                | FlodbaddInterfaceAddrTypeV6::Secured(info)
                | FlodbaddInterfaceAddrTypeV6::Unspecified(info) => (info.ip, info.prefix),
