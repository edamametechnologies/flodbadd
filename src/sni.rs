...
/// Re-exported from [`crate::dns_patterns`] so existing `sni::is_reverse_dns_pattern`
/// callers keep working. The function is pure string analysis with no packet
/// dependency, so it lives in an always-compiled module: this module is gated on
/// `packetcapture` + desktop, and `whitelists.rs` (which is NOT gated) needs it.
pub use crate::dns_patterns::is_reverse_dns_pattern;
